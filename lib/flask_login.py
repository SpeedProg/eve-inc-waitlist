...
class LoginManager(object):
    ...
    def _load_from_cookie(self, cookie):
        ...
        if _request_ctx_stack.top.user is not None:
            app = current_app._get_current_object()
            user_loaded_from_cookie.send(app, user=())
    ...
    def _load_from_header(self, header):
        ...
        if user is not None:
            ...
            app = current_app._get_current_object()
            user_loaded_from_header.send(app, user=())
        else:
            self.reload_user()
    ...
    def _load_from_request(self, request):
        ...
        if user is not None:
            self.reload_user(user=user)
            app = current_app._get_current_object()
            user_loaded_from_request.send(app, user=())
        else:
            self.reload_user()
...
def login_user(user, remember=False, force=False, fresh=True):
    ...
    _request_ctx_stack.top.user = user
    user_logged_in.send(current_app._get_current_object(), user=())
    return True
