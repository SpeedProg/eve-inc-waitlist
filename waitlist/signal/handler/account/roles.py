...
from ... import roles_changed_sig, roles_added_sig
...
def on_roles_changed_check_welcome_mail(_: Any, to_id: int, _: int,
                                        added_roles: Iterable[str],
                                        _: Iterable[str], _: str) -> None:
    """
    Handler to reset welcome mail status.
    """
...
def on_roles_added_create_history_entry(_: Any, by_id: int, role_name: str,
    ...
    note: AccountNote = AccountNote(accountID=by_id, byAccountID=by_id,
                                    note=f'Added role with name{role_name}'
                                    f' and displayName {role_display_name}',
...
def connect() -> None:
    ...
    roles_added_sig.connect(on_roles_added_create_history_entry)
