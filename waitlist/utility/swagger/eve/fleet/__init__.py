...
class EveFleetEndpoint(object):
    ...
    def set_fleet_settings(self, is_free_move, motd):
        ...
        response = client.request(api.op['put_fleets_fleet_id_new_settings'](fleet_id=self.__fleetID, new_settings=settings.get_esi_data()))
    ...
    def get_wings(self):
        ...
        if response.status_code == 200:
            ...
            return EveFleetWings(get_expire_time(response), response.status_code, None, wings)
    ...
    def invite(self, characterID, role, squadID, wingID):
        ...
        response = client.request(api.op['post_fleets_fleet_id_members_invitation'](fleet_id=self.__fleetID, invitation=invite))
        if response.status_code == 204:
            return ESIResponse(get_expire_time(response), response.status_code, None)
        return ESIResponse(get_expire_time(response), response.status_code, response.data['error'])
