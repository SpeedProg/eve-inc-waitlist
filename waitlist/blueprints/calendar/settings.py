...
def get_index():
    ...
    if not perm_manager.getPermission('calendar_event_see_all').can():
        event_query = event_query.filter((CalendarEvent.eventCreatorID == current_user.id) | (CalendarEvent.backseats.any(Account.id == current_user.id)) | (CalendarEvent.organizers.any(Account.id == current_user.id)) )
...
def delete_event_id(event_id):
    ...
    if perm_manager.getPermission('council').can():
        logger.info("%s with id %d is deleting event Title[%s] by Account[%s, %d]", current_user.username, current_user.id, event.eventTitle, event.creator.username, event.creator.id)
        db.session.delete(event)
        db.session.commit()
        return make_response("Event with id["+str(event_id)+"] deleted", 200)

    elif event.creator.id == current_user.id:
        logger.info("%s with id %d is deleting how own event Title[%s]", current_user.username, current_user.id, event.eventTitle)
        db.session.delete(event)
        db.session.commit()
        ...
        return make_response("You are not allowed to delete other peoples events, and this does not seem to be your event!", 403)
