...
def add_item_to_cache(key, item):
    cache[key] = item
...
LIMIT 15;
    ...
    ship_stats15_days = get_query_result('shipStats', ship_stats_query, 2, 3600)
    approved_fits_by_fc_result = get_query_result('approvedFits30Days', approved_fits_by_fc_query, 2, 3600)
    ship_stats1_day = get_query_result('shipStats1Day', ship_stats1_day_query, 2, 3600)

    stats = [
        create_table_cell_row(
            create_table_cell_data('Top 15 approved distinct Hull/Character combinations last 30 days',
                                   ['Hull', 'Amount'], ship_stats15_days),
            create_table_cell_data('15 Most Active Command Core Members over last 7days',
                                   ['Account Name', 'Amount'], approved_fits_by_fc_result, [False, True])
        ),
        create_table_cell_row(
            create_table_cell_data('Top 15 approved distinct Hull/Character combination last 24 hours',
                                   ['Hull', 'Amount'], ship_stats1_day),
            create_table_cell_data('If you have ideas for other stats, use the feedback function.', [], [], [])
        )
    ]
...
def create_table_cell_row(left, right):
    return left, right


def create_table_cell_data(desc, column_names, data, hide_rows=None):
...
def get_query_result(name, query, column_count, cache_time_seconds):
    if has_cache_item(name):
        cache_item = get_cache_item(name)
        ...
        def execute_query(data_name, cc, qq, ct):
            ...
            add_item_to_cache(data_name, create_cache_item(result_, ct))
            db.session.remove()
