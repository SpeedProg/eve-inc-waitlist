    ...
    with db.engine.connect() as con:
        ...
        for table_name in table_names:
            ...
            for column_info in table_columns:
                wl_type, auto_increment = get_type(column_info)
                if auto_increment:
                    columns_with_auto_increment.append([column_info['COLUMN_NAME'], wl_type + " AUTO_INCREMENT"])

                columns.append([column_info['COLUMN_NAME'], wl_type])
            table_columns.close()
        ...
        with Operations.context(mig_context):
            ...
            with op.batch_alter_table('waitlists') as batch:
                batch.create_unique_constraint(constraint_name='uq_waitlists_group_id_waitlist_type',
...
def create_index(name, _, column_name, unique=False):
    if not doFk:
        addTo.create_index(name, column_name, unique=unique)


def unique_constraint(column, name=None):
    if not doFk:
        addTo.create_unique_constraint(constraint_name=name, columns=[column])


def primary_key_constraint(*columns, name=None):
    if not doFk:
        addTo.create_primary_key(name, columns)


def foreign_key_constraint(own_columns: List[str], other_ids: List[str], name=None, onupdate=None, ondelete=None):
    if doFk:
...
def add_keys_and_constraits():
    ...
    with op.batch_alter_table('account_notes') as batch:
        ...
        if doFk:
            ...
            primary_key_constraint('entry_id', name=op.f('pk_account_notes'))
    ...
    with op.batch_alter_table('accounts') as batch:
        addTo = batch
        foreign_key_constraint(['current_char'], ['characters.id'],
                               name=op.f('fk_accounts_current_char_characters'))
        primary_key_constraint('id', name=op.f('pk_accounts'))
        unique_constraint('login_token', name=op.f('uq_accounts_login_token'))
        unique_constraint('username', name=op.f('uq_accounts_username'))

    with op.batch_alter_table('apicache_allianceinfo') as batch:
        addTo = batch
        primary_key_constraint('id', name=op.f('pk_apicache_allianceinfo'))
        if not doFk:
    ...
    with op.batch_alter_table('apicache_characteraffiliation') as batch:
        addTo = batch
        primary_key_constraint('id', name=op.f('pk_apicache_characteraffiliation'))
    ...
    with op.batch_alter_table('apicache_characterinfo') as batch:
        addTo = batch
        primary_key_constraint('id', name=op.f('pk_apicache_characterinfo'))
    ...
    with op.batch_alter_table('apicache_corporationinfo') as batch:
        addTo = batch
        primary_key_constraint('id', name=op.f('pk_apicache_corporationinfo'))
    ...
    with op.batch_alter_table('backseats') as batch:
        addTo = batch
        foreign_key_constraint(['account_id'], ['accounts.id'], name=op.f('fk_backseats_account_id_accounts'),
                               ondelete='CASCADE'),
        foreign_key_constraint(['group_id'], ['waitlist_groups.group_id'],
                               name=op.f('fk_backseats_group_id_waitlist_groups'), ondelete='CASCADE')

    with op.batch_alter_table('ban') as batch:
        addTo = batch
        foreign_key_constraint(['admin'], ['characters.id'], name=op.f('fk_ban_admin_characters')),
        primary_key_constraint('id', name=op.f('pk_ban'))
        create_index(op.f('ix_ban_name'), 'ban', ['name'], unique=False)

    with op.batch_alter_table('calendar_backseat') as batch:
        addTo = batch
        foreign_key_constraint(['account_id'], ['accounts.id'],
                               name=op.f('fk_calendar_backseat_account_id_accounts'), onupdate='CASCADE',
                               ondelete='CASCADE'),
        foreign_key_constraint(['event_id'], ['calendar_event.event_id'],
    ...
    with op.batch_alter_table('calendar_category') as batch:
        addTo = batch
        primary_key_constraint('category_id', name=op.f('pk_calendar_category'))
    ...
    with op.batch_alter_table('calendar_event') as batch:
        addTo = batch
        foreign_key_constraint(['approver_id'], ['accounts.id'], name=op.f('fk_calendar_event_approver_id_accounts'),
                               onupdate='CASCADE', ondelete='CASCADE')
        foreign_key_constraint(['event_category_id'], ['calendar_category.category_id'],
                               name=op.f('fk_calendar_event_event_category_id_calendar_category'), onupdate='CASCADE',
                               ondelete='CASCADE')
        foreign_key_constraint(['event_creator_id'], ['accounts.id'],
                               name=op.f('fk_calendar_event_event_creator_id_accounts'), onupdate='CASCADE',
                               ondelete='CASCADE')
        primary_key_constraint('event_id', name=op.f('pk_calendar_event'))
    ...
    with op.batch_alter_table('calendar_organizer') as batch:
        addTo = batch
        foreign_key_constraint(['account_id'], ['accounts.id'], name=op.f('fk_calendar_organizer_account_id_accounts'),
                               onupdate='CASCADE', ondelete='CASCADE')
        foreign_key_constraint(['event_id'], ['calendar_event.event_id'],
    ...
    with op.batch_alter_table('ccvote') as batch:
        addTo = batch
        foreign_key_constraint(['fcvote_id'], ['accounts.id'], name=op.f('fk_ccvote_fcvote_id_accounts'))
        foreign_key_constraint(['lmvote_id'], ['accounts.id'], name=op.f('fk_ccvote_lmvote_id_accounts'))
        foreign_key_constraint(['voter_id'], ['characters.id'], name=op.f('fk_ccvote_voter_id_characters'))
        primary_key_constraint('ccvote_id', name=op.f('pk_ccvote'))

    with op.batch_alter_table('characters') as batch:
        addTo = batch
        primary_key_constraint('id', name=op.f('pk_characters'))

    with op.batch_alter_table('comp_history') as batch:
        addTo = batch
        foreign_key_constraint(['source_id'], ['accounts.id'], name=op.f('fk_comp_history_source_id_accounts'))
        foreign_key_constraint(['target_id'], ['characters.id'], name=op.f('fk_comp_history_target_id_characters'))
        primary_key_constraint('history_id', name=op.f('pk_comp_history'))
    ...
    with op.batch_alter_table('comp_history_ext_inv') as batch:
        addTo = batch
        foreign_key_constraint(['history_id'], ['comp_history.history_id'],
                               name=op.f('fk_comp_history_ext_inv_history_id_comp_history'))
        foreign_key_constraint(['waitlist_id'], ['waitlists.id'],
                               name=op.f('fk_comp_history_ext_inv_waitlist_id_waitlists'))
        primary_key_constraint('invite_ext_id', name=op.f('pk_comp_history_ext_inv'))

    with op.batch_alter_table('comp_history_fits') as batch:
        addTo = batch
        foreign_key_constraint(['fit_id'], ['fittings.id'], name=op.f('fk_comp_history_fits_fit_id_fittings'))
        foreign_key_constraint(['history_id'], ['comp_history.history_id'],
                               name=op.f('fk_comp_history_fits_history_id_comp_history'))
        primary_key_constraint('id', name=op.f('pk_comp_history_fits'))

    with op.batch_alter_table('constellation') as batch:
        addTo = batch
        primary_key_constraint('constellation_id', name=op.f('pk_constellation'))
    ...
    with op.batch_alter_table('crest_fleets') as batch:
        addTo = batch
        foreign_key_constraint(['comp_id'], ['accounts.id'], name=op.f('fk_crest_fleets_comp_id_accounts'))
        foreign_key_constraint(['group_id'], ['waitlist_groups.group_id'],
                               name=op.f('fk_crest_fleets_group_id_waitlist_groups'))
        primary_key_constraint('fleet_id', name=op.f('pk_crest_fleets'))

    with op.batch_alter_table('eveapiscope') as batch:
        addTo = batch
        primary_key_constraint('scope_id', name=op.f('pk_eveapiscope'))
    ...
    with op.batch_alter_table('event_history_entries') as batch:
        addTo = batch
        foreign_key_constraint(['type_id'], ['event_history_types.type_id'],
                               name=op.f('fk_event_history_entries_type_id_event_history_types'))
        primary_key_constraint('history_id', name=op.f('pk_event_history_entries'))
    ...
    with op.batch_alter_table('event_history_info') as batch:
        addTo = batch
        foreign_key_constraint(['history_id'], ['event_history_entries.history_id'],
                               name=op.f('fk_event_history_info_history_id_event_history_entries'))
        primary_key_constraint('info_id', name=op.f('pk_event_history_info'))

    with op.batch_alter_table('event_history_types') as batch:
        addTo = batch
        primary_key_constraint('type_id', name=op.f('pk_event_history_types'))
        unique_constraint('type_name', name=op.f('uq_event_history_types_type_name'))

    with op.batch_alter_table('fcs') as batch:
        addTo = batch
        foreign_key_constraint(['account_id'], ['accounts.id'], name=op.f('fk_fcs_account_id_accounts'),
                               ondelete='CASCADE')
        foreign_key_constraint(['group_id'], ['waitlist_groups.group_id'],
    ...
    with op.batch_alter_table('feedback') as batch:
        addTo = batch
        foreign_key_constraint(['user'], ['characters.id'], name=op.f('fk_feedback_user_characters'))
        primary_key_constraint('id', name=op.f('pk_feedback'))
    ...
    with op.batch_alter_table('fit_module') as batch:
        addTo = batch
        foreign_key_constraint(['fit_id'], ['fittings.id'], name=op.f('fk_fit_module_fit_id_fittings'))
        foreign_key_constraint(['module_id'], ['invtypes.type_id'], name=op.f('fk_fit_module_module_id_invtypes'))
        primary_key_constraint('fit_id', 'module_id', name=op.f('pk_fit_module'))

    with op.batch_alter_table('fittings') as batch:
        addTo = batch
        foreign_key_constraint(['ship_type'], ['invtypes.type_id'], name=op.f('fk_fittings_ship_type_invtypes'))
        primary_key_constraint('id', name=op.f('pk_fittings'))

    with op.batch_alter_table('fleetmanager') as batch:
        addTo = batch
        foreign_key_constraint(['account_id'], ['accounts.id'], name=op.f('fk_fleetmanager_account_id_accounts'),
                               ondelete='CASCADE')
        foreign_key_constraint(['group_id'], ['waitlist_groups.group_id'],
    ...
    with op.batch_alter_table('incursion_layout') as batch:
        addTo = batch
        foreign_key_constraint(['constellation'], ['constellation.constellation_id'],
                               name=op.f('fk_incursion_layout_constellation_constellation'))
        foreign_key_constraint(['dockup'], ['station.station_id'],
                               name=op.f('fk_incursion_layout_dockup_station'))
        foreign_key_constraint(['headquarter'], ['solarsystem.solar_system_id'],
                               name=op.f('fk_incursion_layout_headquarter_solarsystem'))
        foreign_key_constraint(['staging'], ['solarsystem.solar_system_id'],
                               name=op.f('fk_incursion_layout_staging_solarsystem'))
        primary_key_constraint('constellation', name=op.f('pk_incursion_layout'))

    with op.batch_alter_table('invmarketgroups') as batch:
        addTo = batch
        foreign_key_constraint(['parent_group_id'], ['invmarketgroups.market_group_id'],
                               name=op.f('fk_invmarketgroups_parent_group_id_invmarketgroups'))
        primary_key_constraint('market_group_id', name=op.f('pk_invmarketgroups'))

    with op.batch_alter_table('invtypes') as batch:
        addTo = batch
        primary_key_constraint('type_id', name=op.f('pk_invtypes'))
        create_index(op.f('ix_invtypes_group_id'), 'invtypes', ['group_id'], unique=False)

    with op.batch_alter_table('linked_chars') as batch:
        addTo = batch
        foreign_key_constraint(['char_id'], ['characters.id'], name=op.f('fk_linked_chars_char_id_characters'),
                               onupdate='CASCADE', ondelete='CASCADE')
        foreign_key_constraint(['id'], ['accounts.id'], name=op.f('fk_linked_chars_id_accounts'),
                               onupdate='CASCADE', ondelete='CASCADE')

    with op.batch_alter_table('permission_roles') as batch:
        addTo = batch
        foreign_key_constraint(['permission'], ['permissions.id'],
                               name=op.f('fk_permission_roles_permission_permissions'))
        foreign_key_constraint(['role'], ['roles.id'], name=op.f('fk_permission_roles_role_roles'))

    with op.batch_alter_table('permissions') as batch:
        addTo = batch
        primary_key_constraint('id', name=op.f('pk_permissions'))
        unique_constraint('name', name=op.f('uq_permissions_name'))

    with op.batch_alter_table('role_changes') as batch:
        addTo = batch
        foreign_key_constraint(['entry_id'], ['account_notes.entry_id'],
                               name=op.f('fk_role_changes_entry_id_account_notes'), onupdate='CASCADE',
                               ondelete='CASCADE')
        foreign_key_constraint(['role_id'], ['roles.id'], name=op.f('fk_role_changes_role_id_roles'),
                               onupdate='CASCADE', ondelete='CASCADE')
        primary_key_constraint('role_change_id', name=op.f('pk_role_changes'))

    with op.batch_alter_table('roles') as batch:
        addTo = batch
        primary_key_constraint('id', name=op.f('pk_roles'))
        unique_constraint('name', name=op.f('uq_roles_name'))

    with op.batch_alter_table('settings') as batch:
        addTo = batch
        primary_key_constraint('key', name=op.f('pk_settings'))

    with op.batch_alter_table('solarsystem') as batch:
        addTo = batch
        primary_key_constraint('solar_system_id', name=op.f('pk_solarsystem'))
    ...
    with op.batch_alter_table('ssotoken') as batch:
        addTo = batch
        foreign_key_constraint(['account_id'], ['accounts.id'], name=op.f('fk_ssotoken_account_id_accounts'),
                               onupdate='CASCADE', ondelete='CASCADE')
        primary_key_constraint('account_id', name=op.f('pk_ssotoken'))

    with op.batch_alter_table('station') as batch:
        addTo = batch
        primary_key_constraint('station_id', name=op.f('pk_station'))
        create_index(op.f('ix_station_station_name'), 'station', ['station_name'], unique=True)

    with op.batch_alter_table('tickets') as batch:
        addTo = batch
        foreign_key_constraint(['character_id'], ['characters.id'],
                               name=op.f('fk_tickets_character_id_characters'))
        primary_key_constraint('id', name=op.f('pk_tickets'))
    ...
    with op.batch_alter_table('tokenscope') as batch:
        addTo = batch
        foreign_key_constraint(['scope_id'], ['eveapiscope.scope_id'],
                               name=op.f('fk_tokenscope_scope_id_eveapiscope'), onupdate='CASCADE',
                               ondelete='CASCADE')
        foreign_key_constraint(['token_id'], ['ssotoken.account_id'], name=op.f('fk_tokenscope_token_id_ssotoken'),
                               onupdate='CASCADE', ondelete='CASCADE')
        primary_key_constraint('token_id', 'scope_id', name=op.f('pk_tokenscope'))
    ...
    with op.batch_alter_table('trivia') as batch:
        ...
        foreign_key_constraint(['created_by_id'], ['accounts.id'], name=op.f('fk_trivia_created_by_id_accounts'))
        primary_key_constraint('trivia_id', name=op.f('pk_trivia'))

    with op.batch_alter_table('trivia_answer') as batch:
        addTo = batch
        foreign_key_constraint(['question_id'], ['trivia_question.question_id'],
                               name=op.f('fk_trivia_answer_question_id_trivia_question'))
        primary_key_constraint('answer_id', 'question_id', name=op.f('pk_trivia_answer'))

    with op.batch_alter_table('trivia_question') as batch:
        addTo = batch
        foreign_key_constraint(['trivia_id'], ['trivia.trivia_id'],
                               name=op.f('fk_trivia_question_trivia_id_trivia'))
        primary_key_constraint('question_id', name=op.f('pk_trivia_question'))
        if not doFk:
    ...
    with op.batch_alter_table('trivia_submission') as batch:
        addTo = batch
        foreign_key_constraint(['submittor_account_id'], ['accounts.id'],
                               name=op.f('fk_trivia_submission_submittor_account_id_accounts'))
        foreign_key_constraint(['submittor_id'], ['characters.id'],
                               name=op.f('fk_trivia_submission_submittor_id_characters'))
        foreign_key_constraint(['trivia_id'], ['trivia.trivia_id'],
                               name=op.f('fk_trivia_submission_trivia_id_trivia'))
        primary_key_constraint('submission_id', name=op.f('pk_trivia_submission'))
    ...
    with op.batch_alter_table('trivia_submission_answer') as batch:
        ...
        foreign_key_constraint(['submission_id'], ['trivia_submission.submission_id'],
                               name=op.f('fk_trivia_submission_answer_submission_id_trivia_submission'))
        primary_key_constraint('submission_id', 'question_id', name=op.f('pk_trivia_submission_answer'))

    with op.batch_alter_table('ts_dati') as batch:
        addTo = batch
        primary_key_constraint('teamspeak_id', name=op.f('pk_ts_dati'))

    with op.batch_alter_table('waitlist_entries') as batch:
        addTo = batch
        foreign_key_constraint(['user'], ['characters.id'], name=op.f('fk_waitlist_entries_user_characters'))
        foreign_key_constraint(['waitlist_id'], ['waitlists.id'],
                               name=op.f('fk_waitlist_entries_waitlist_id_waitlists'), onupdate='CASCADE',
                               ondelete='CASCADE')
        primary_key_constraint('id', name=op.f('pk_waitlist_entries'))

    with op.batch_alter_table('waitlist_entry_fits') as batch:
        addTo = batch
        foreign_key_constraint(['entry_id'], ['waitlist_entries.id'],
                               name=op.f('fk_waitlist_entry_fits_entry_id_waitlist_entries'), onupdate='CASCADE',
                               ondelete='CASCADE')
        foreign_key_constraint(['fit_id'], ['fittings.id'], name=op.f('fk_waitlist_entry_fits_fit_id_fittings'),
                               onupdate='CASCADE', ondelete='CASCADE')
        primary_key_constraint('fit_id', name=op.f('pk_waitlist_entry_fits'))

    with op.batch_alter_table('waitlist_groups') as batch:
        addTo = batch
        foreign_key_constraint(['constellation_id'], ['constellation.constellation_id'],
                               name=op.f('fk_waitlist_groups_constellation_id_constellation'))
        foreign_key_constraint(['dockup_id'], ['station.station_id'],
        ...
        """
        foreign_key_constraint(['system_id'], ['solarsystem.solar_system_id'],
                               name=op.f('fk_waitlist_groups_system_id_solarsystem'))
        primary_key_constraint('group_id', name=op.f('pk_waitlist_groups'))
        unique_constraint('display_name', name=op.f('uq_waitlist_groups_display_name'))
        unique_constraint('group_name', name=op.f('uq_waitlist_groups_group_name'))

    with op.batch_alter_table('waitlists') as batch:
        addTo = batch
        foreign_key_constraint(['group_id'], ['waitlist_groups.group_id'],
                               name=op.f('fk_waitlists_group_id_waitlist_groups'))
        primary_key_constraint('id', name=op.f('pk_waitlists'))

    with op.batch_alter_table('whitelist') as batch:
        addTo = batch
        foreign_key_constraint(['admin_id'], ['characters.id'], name=op.f('fk_whitelist_admin_id_characters'))
        foreign_key_constraint(['character_id'], ['characters.id'],
                               name=op.f('fk_whitelist_character_id_characters'))
        primary_key_constraint('character_id', name=op.f('pk_whitelist'))
